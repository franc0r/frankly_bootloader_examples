 ...
 * @brief Source file of bootloader API for RP2040 Pico
 * @version 1.0
 * @date 2025-09-14
 *
...
/*
 * This array contains the device identification information, which is stored in flash memory.
 * For RP2040, we use flash storage for device identification.
 * The default value is 0xFFFFFFFFU for uninitialized flash.
 */
...
/**
 * @brief Block until message is received from UART
 */
  ...
  for (;;) {
    ...
    if (req_autostart) {
      ...
      if (uart_is_readable(uart0)) {
        buffer[buffer_idx] = uart_getc(uart0);
        buffer_idx++;
...
/**
 * @brief Transmit response over UART
 */
static void transmitResponse(const msg::Msg& response) {
  ...
  for (uint32_t buffer_idx = 0U; buffer_idx < buffer.size(); buffer_idx++) {
    uart_putc_raw(uart0, buffer.at(buffer_idx));
  }
}

// Software CRC32 implementation (RP2040 has no hardware CRC) --------------------------------------------------------
...
extern "C" void FRANKLYBOOT_Run(void) {
  ...
  // Check if autostart shall be disabled by app firmware
  const bool autostart_disable = (autoboot_disable_flag == AUTOBOOT_DISABLE_OVERRIDE_KEY);
  autoboot_disable_flag = 0;  // Reset flag
...
[[nodiscard]] uint32_t hwi::getUniqueIDWord(const uint32_t idx) {
  ...
  uint32_t uid_value = 0U;
  if (idx < 2U) {
    // RP2040 has 64-bit unique ID, split into two 32-bit words
    uint32_t* uid_ptr = (uint32_t*)board_id.id;
    uid_value = uid_ptr[idx];
  }
...
uint32_t hwi::calculateCRC(uint32_t src_address, uint32_t num_bytes) {
  // RP2040 has no hardware CRC, use software implementation
...
bool hwi::eraseFlashPage(uint32_t page_id) {
  // Calculate flash offset (page_id * page_size)
  uint32_t flash_offset = page_id * FLASH_SECTOR_SIZE;
...
bool hwi::writeDataBufferToFlash(uint32_t dst_address, uint32_t dst_page_id, uint8_t* src_data_ptr,
                                 uint32_t num_bytes) {
  // Check if data size is valid (must be multiple of page size)
  if ((num_bytes % FLASH_PAGE_SIZE) != 0) {
    return false;
  }

  // Calculate flash offset from XIP base
